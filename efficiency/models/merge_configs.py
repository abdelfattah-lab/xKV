...
class xKVConfig:
    ...
    def to_yaml(self, path: str):
        """
        Dump this config to a YAML file under the key 'palu_config'.
        ...
        with open(path, "w") as f:
            yaml.safe_dump({"palu_config": data}, f, sort_keys=False)
...
def generate_consecutive_palu_config(
    layer_merge_impl: str = "svd",
    start_layer: int = 0,
    end_layer: int = -1,
    num_layers: Optional[int] = None,
    group_size: int = 2,
    rank_k: Optional[int] = 192,
    rank_v: Optional[int] = 288,
    slerp_t: float = 0.5,
    slerp_gamma: float = 1.0,
    merge_key: bool = True,
    merge_value: bool = True,
...
if __name__ == "__main__":
    ...
    cfg_svd = generate_consecutive_palu_config(
        layer_merge_impl="svd",
        start_layer=0,
        end_layer=31,
        group_size=2,
