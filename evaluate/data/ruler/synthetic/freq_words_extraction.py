...
def generate_input_output(max_len, num_words=-1, coded_wordlen=6, vocab_size=2000, incremental=10, alpha=2.0):
    ...
    if context_end_marker in template:
        ...
        # Fallback: try with space after context
        context_end_marker = "{context} "
        if context_end_marker in template:
            split_idx = template.index(context_end_marker) + len(context_end_marker)
            context_template = template[:split_idx]
            ...
            # Last fallback: use {context} without any suffix
            context_end_marker = "{context}"
            split_idx = template.index(context_end_marker) + len(context_end_marker)
            context_template = template[:split_idx]
            query_template = template[split_idx:]
    ...
    def gen_text(num_words):
        ...
        context_content = ' '.join(sampled_words)

        # Format context and query separately
        formatted_context = context_template.format(context=context_content)
        formatted_query = query_template.format(query='')
        input_text = formatted_context + formatted_query

        return input_text, formatted_context, formatted_query, vocab[1:4]

    if num_words > 0:
        num_words = num_words
        text, context, query, answer = gen_text(num_words)
        ...
        num_words -= incremental
    text, context, query, answer = gen_text(num_words)
    return text, context, query, answer, num_words
...
def sys_kwext(num_samples: int, max_seq_length: int, incremental: int = 10):
    ...
    _, _, _, _, num_example_words = generate_input_output(input_max_len,
    ...
    for index in tqdm(range(num_samples)):
        ...
        # construct input
        input_max_len = max_seq_length
        input_text, context, query, answer, _ = generate_input_output(input_max_len,
