...
# KIVI-2 (comp ~6.40×)
CUDA_VISIBLE_DEVICES=0,1,2,3 OMP_NUM_THREADS=48 torchrun --standalone --nnodes=1 --nproc_per_node 4 evaluate/eval_acc.py \
    --dataset_name "$DATASET" --model_name_or_path $MODEL --datalen 65536 --batch_size 1 --flash2 --method kivi \
    | tee -a logs/ruler_qwen/kivi.log

# PyramidKV (comp ~6.00×)
CUDA_VISIBLE_DEVICES=0,1,2,3 OMP_NUM_THREADS=48 torchrun --standalone --nnodes=1 --nproc_per_node 4 evaluate/eval_acc.py \
    --dataset_name "$DATASET" --model_name_or_path $MODEL --datalen 65536 --batch_size 1 --flash2 --method pyramidkv \
    | tee -a logs/ruler_qwen/pyramidkv.log

# SnapKV (comp ~6.00×)
CUDA_VISIBLE_DEVICES=0,1,2,3 OMP_NUM_THREADS=48 torchrun --standalone --nnodes=1 --nproc_per_node 4 evaluate/eval_acc.py \
    --dataset_name "$DATASET" --model_name_or_path $MODEL --datalen 65536 --batch_size 1 --flash2 --method snapkv \
