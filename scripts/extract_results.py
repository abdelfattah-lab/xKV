...
EXPERIMENT_ORDER = ['full', 'MiniCache', 'StreamingLLM', 'SnapKV', 'PyramidKV', 'KIVI', 'Quest', 'shadowkv_rank', 'xkey', 'xkv']
...
RULER_DATASET_ORDER: List[str] = [
    ...
    'qa_1', 'qa_2', 'vt', 'fwe', 'niah_multiturn_1', 'niah_multiturn_2'
]
RULER_DATASET_NAMES: Dict[str, str] = {
    ...
    'niah_multiturn_1': 'N-MT1', 'niah_multiturn_2': 'N-MT2',
}
...
def sort_rank_variants(labels: List[str], descending: bool = False) -> List[str]:
    """Sort variant labels by their numeric rank component.

    Handles labels like:
      - 'shadowkv_rank-128_sparse-2048'
      - 'xkey-1_rank-128_sparse-2048'

    If a label has no rank component, it is placed after those with rank.
    Sort order is ascending by default (small rank first); set descending=True for reverse.
    """
    items: List[Tuple[int | float, str]] = []
    for lbl in labels:
        m = re.search(r"rank[-_]?([0-9]+)", lbl, flags=re.IGNORECASE)
        if m:
            try:
                r = int(m.group(1))
            except ValueError:
            ...
            r = float('inf')
        items.append((r, lbl))
    items.sort(key=lambda t: (-(t[0]) if descending and isinstance(t[0], (int, float)) else t[0], t[1]))
...
def build_and_save_table(bench: str, frames: List[pd.DataFrame]):
    ...
    for exp in EXPERIMENT_ORDER:
        ...
        # Insert shadowkv_rank-* variants sorted by rank
        if exp == 'shadowkv_rank':
            shadowkv_labels = [idx for idx in table.index if idx.lower().startswith('shadowkv_rank-')]
            if shadowkv_labels:
                shadowkv_sorted = sort_rank_variants(shadowkv_labels)
                for lbl in shadowkv_sorted:
                    if lbl not in experiment_order:
                        experiment_order.append(lbl)
        if exp == 'xKV' and xkv_variants_sorted:
            experiment_order.extend([v for v in xkv_variants_sorted if v not in experiment_order])
        # Insert xkey-* variants sorted by rank
        if exp == 'xkey':
            xkey_labels = [idx for idx in table.index if idx.lower().startswith('xkey-')]
            if xkey_labels:
                xkey_sorted = sort_rank_variants(xkey_labels)
                for lbl in xkey_sorted:
                    if lbl not in experiment_order:
                        experiment_order.append(lbl)
        if exp == 'KIVI':
