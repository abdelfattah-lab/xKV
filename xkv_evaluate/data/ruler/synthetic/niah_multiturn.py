...
parser.add_argument("--template_context", type=str, required=True)
parser.add_argument("--template_query", type=str, required=True)
...
def generate_random_long():
    key_len = 256
    # Initialize an empty list to hold the UUID strings
    uuid_list = []
    # Keep generating UUIDs until the total length is at least key_len characters
    total_length = 0
    while total_length < key_len:
        new_uuid = str(uuid.uuid4())
        uuid_list.append(new_uuid)
        total_length += len(new_uuid)

    # Join the UUIDs and trim to key_len characters if necessary
    uuid_string = ''.join(uuid_list)[:key_len]
    return uuid_string
...
def generate_input_output(num_haystack):
    ...
    answers = [values[i] for i in indices]
    ...
    return input_text, queries_list, answers
